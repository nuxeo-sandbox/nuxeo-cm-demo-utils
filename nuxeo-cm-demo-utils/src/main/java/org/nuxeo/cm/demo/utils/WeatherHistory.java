...
* @since 5.8
*/
/** This operation receives the name of a city and a date (format YYYY-MM-DDTHH:MM:SS),
 *  and fills a specific field of the current document with weather informations.
 *
 ...
 *      1/ Typical POC demo. We don't do a lot of error-check here,
 *         to let the code as much readable as possible: No check
 *         on the JSON nodes, no check if the schema is valid, ...
 *
 *      2/ Also: We update a specific field in a specific schema, and then we save
 *         the document
 *         => Current document *must* exist and *must* have a
 *            String field incl:incident_weather
 *
 *      3/ You need external system keys to get:
 *          -> The latitude/longitude given the name of a city
 *              => Done with a BING key (we use dev.virtualearth.net service)
 *          -> The weather info for this location at this date
 *              => Done with a forecast.io key
 *
 *          To avoid hardcoding the keys in this code, we use configuration entries
 *          in nuxeo.conf. So you need to add the following to nuxeo.conf of the
 *          server:
 *              demo.weatherhistory.forecastio.key=YOUR forecast.io KEY HERE
 *              demo.weatherhistory.bing.key=YOUR Bing KEY HERE
 *          (and of course, restart the server after changing the conf.)
 *
 *          Getting a dev. code from these services is easy and free (as of today, 2013-11-11)
 *
 *
 * The super-amazing ;-> algorithm is:
 *      1/ Get the latitude/longitude for a city
 *      2/ Build the forecast URL
 *      3/ Connect and get the data
 *      4/ Extract data from the JSON string
 *      5/ Update the incl:incident_weather property of the document
 */
...
public class WeatherHistory {
    ...
    */
    ...
    public DocumentModel run(DocumentModel input) throws PropertyException, ClientException, IOException {
        ...
        if(!_devKeysOK) {
            throw new ClientException("forecast.io key and/or bing key are missing or invalid. Are they correctly set-up in nuxeo.conf?");
        }

        if(   city != null && city != ""
           && dateStr != null && dateStr != "") {// We don' check if the date is well formatted
